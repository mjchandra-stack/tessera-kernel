...
fn run() -> u64 {
    ...
    // kernel wrote are the SDK's; what an input request means is this driver's.
    ...
    match served {
        ...
        // no other exit before the SDK gave it one, so reaching here at all is
        // worth reporting rather than reporting success.
        Ok(()) => fail(0xda, 11),
        Err(_) => fail(0xda, 1),
    }
}
